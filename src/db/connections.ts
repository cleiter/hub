...
export class ConnectionRepository {
  ...
  /**
   * The token is generated by the caller and only its hash reaches the database, so a leaked
   * dump cannot be replayed against the webhook endpoint.
   */
  async createGitLab(input: CreateGitLabConnectionInput): Promise<GitLabConnectionRecord> {
    const [row] = await this.database
      ...
      .returning();
    if (row === undefined) throw new Error("gitlab connection insert returned no row");
    return gitlabConnection(row);
  }

  /** Returns false when the connection does not belong to the organization, so callers cannot
   * rotate another organization's token by guessing an id. */
  async rotateGitLabToken(input: RotateGitLabConnectionTokenInput): Promise<boolean> {
  ...
  async findGitLabSecret(
    ...
    return row === undefined
      ? undefined
      : { connection: gitlabConnection(row), tokenHash: row.tokenHash };
  }

  async listGitLabForOrganization(organizationId: string): Promise<GitLabConnectionRecord[]> {
    const rows = await this.database
      .select()
      .from(schema.gitlabConnections)
      .where(eq(schema.gitlabConnections.organizationId, organizationId))
      .orderBy(schema.gitlabConnections.label, schema.gitlabConnections.id);
    return rows.map(gitlabConnection);
  }
